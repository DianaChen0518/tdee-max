...
<template>
  ...
  >
    <div
      class="bg-white dark:bg-[#121212] p-6 rounded-card border border-gray-100 dark:border-[#333] w-full max-w-5xl shadow-premium transition-all flex flex-col h-[95vh] md:h-[90vh]"
    >
      <!-- Header -->
      <div class="flex flex-wrap justify-between items-center mb-6 shrink-0 gap-4">
        ...
        <div class="flex gap-2 items-center bg-gray-100 dark:bg-[#252525] p-1 rounded-btn">
          <button
            @click="setRange('7')"
            :class="[
              'px-4 py-1.5 text-xs font-bold rounded-btn transition-all',
              timeRange === '7'
        ...
        <button
          @click="
            ...
            emit('close');
          "
          class="text-gray-500 hover:text-red-500 transition-colors bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-btn font-bold text-sm"
        >
          {{ t('datavis.close') }}
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6 shrink-0">
        <div
          class="bg-blue-50/50 dark:bg-blue-900/10 p-5 rounded-inner border border-blue-100 dark:border-blue-900/30 text-center transition-colors shadow-sm"
        >
          <div class="text-[10px] uppercase tracking-wider text-blue-600 dark:text-blue-400 mb-1 font-black">
            {{ t('datavis.stats.totalDeficit') }}
          </div>
          <div
            :class="[
              'text-3xl font-black',
          ...
          >
            ...
            <span class="text-sm font-normal opacity-70">kcal</span>
          </div>
        </div>
        <div
          class="bg-green-50/50 dark:bg-green-900/10 p-5 rounded-inner border border-green-100 dark:border-green-900/30 text-center transition-colors shadow-sm"
        >
          <div class="text-[10px] uppercase tracking-wider text-green-700 dark:text-green-500 mb-1 font-black">
            {{ t('datavis.stats.fatChange') }}
          </div>
          <div
            :class="[
              'text-3xl font-black',
              summaryStats.totalDeficit > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'
            ]"
          >
        ...
        <div
          class="bg-orange-50/50 dark:bg-orange-900/10 p-5 rounded-inner border border-orange-100 dark:border-orange-900/30 text-center transition-colors shadow-sm"
        >
          <div class="text-[10px] uppercase tracking-wider text-orange-700 dark:text-orange-500 mb-1 font-black">
            {{ t('datavis.stats.avgIntake') }}
          </div>
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-6">
        ...
        <template v-else>
          <!-- Weight Trend Chart -->
          <div
            class="bg-gray-50/30 dark:bg-[#1e1e1e] p-5 rounded-card border border-gray-100 dark:border-[#333] transition-colors relative min-h-[300px] shadow-sm"
          >
            <h3
              class="text-xs font-black text-gray-500 dark:text-gray-400 mb-4 flex items-center gap-2 uppercase tracking-widest"
            >
          ...
          <div class="mt-4 pb-4">
            <h3 class="text-[10px] font-black text-gray-400 mb-3 uppercase tracking-widest">
              {{ t('datavis.details') }}
            </h3>
            <div class="flex gap-3 overflow-x-auto pb-4 custom-scrollbar">
              <div
                ...
                :key="r.date"
                class="bg-white dark:bg-[#252525] p-3 rounded-inner text-[10px] min-w-[140px] shrink-0 border border-gray-100 dark:border-[#444] shadow-sm hover:border-blue-400 transition-colors cursor-default"
              >
                ...
                <div class="text-gray-500 font-bold tracking-tight">
                  {{ r.weight }} <span class="font-normal opacity-70">kg</span>
                </div>
              </div>
            </div>
          </div>
