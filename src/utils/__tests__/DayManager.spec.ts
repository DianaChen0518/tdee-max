...
describe('DayManager', () => {
  ...
  describe('findLastDietDayFoods', () => {
    ...
    it('should find foods from the most recent past day with food logs', () => {
      const foods: Food[] = [
        { name: 'Rice', cals: 200, multiplier: 1, mealType: 'lunch' }
      ];
      const db: Database = {
    ...
    it('should deep copy foods (no reference leakage)', () => {
      const foods: Food[] = [
        { id: 'old-id', name: 'Egg', cals: 70, multiplier: 1, mealType: 'breakfast' }
      ];
      const db: Database = {
  ...
  describe('copyFoods', () => {
    ...
    it('should accumulate multiplier for duplicate foods in target', () => {
      const source: Food[] = [
        { id: 's1', name: 'Egg', cals: 70, multiplier: 2, mealType: 'breakfast' }
      ];
      const target: Food[] = [
        { id: 't1', name: 'Egg', cals: 70, multiplier: 1, mealType: 'breakfast' }
      ];
    ...
    it('should add new food entries with fresh IDs when not duplicated', () => {
      const source: Food[] = [
        { id: 'src-id', name: 'Apple', cals: 52, multiplier: 1, mealType: 'lunch' }
      ];
    ...
    it('should return unchanged target if no matching foods', () => {
      const source: Food[] = [
        { id: 's1', name: 'Steak', cals: 400, multiplier: 1, mealType: 'dinner' }
      ];
      const target: Food[] = [
        { id: 't1', name: 'Bread', cals: 100, multiplier: 1, mealType: 'breakfast' }
      ];
      const result = DayManager.copyFoods(source, target, 'breakfast');
    ...
    it('should not mutate original source or target arrays', () => {
      const source: Food[] = [
        { id: 's1', name: 'Milk', cals: 50, multiplier: 1, mealType: 'breakfast' }
      ];
      const target: Food[] = [];
      const result = DayManager.copyFoods(source, target, 'breakfast');
